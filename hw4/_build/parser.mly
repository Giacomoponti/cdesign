...
%nonassoc BANG
%nonassoc TILDE
%nonassoc LBRACKET RBRACKET (* are this useful ?*) 
...
stmt: 
  ...
  | FOR LPAREN d=separated_list(COMMA, vdecl) SEMI exp_opt= option(exp) SEMI stmt=option(stmt) RPAREN b=block 
                        { loc $startpos $endpos @@ For(d, exp_opt, stmt, b)}
block:
